...
void calScale30_50(){
   ...
   TH1F *hData;
   f1->GetObject("Table 5/Hist1D_y1", hData);

   TFile *f2 = new TFile("genPt_reRECO.root");
   TH1D* hGenPt;
   f2->GetObject("hGenPt", hGenPt);

   TH3D* hScale = new TH3D("hScale", "30-50", 60, 0, 3, 100, 0, 10, 60, 1.7, 2.0);
   ...
   scale_factor_perEvt.push_back(yield_data_Pt7_8GeV_perEvt / hGenPt->Integral(81, 100)); // 8.0 - 10.0 GeV
   ...
   TFile fout("hscale30_50.root", "recreate");
   hScale->Write();
}
