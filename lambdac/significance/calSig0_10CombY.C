...
void calSig0_10CombY()
{
   ...
   f4->GetObject("hMassVsPtVsYCentMtd", hBkgMtd);
   ...
   //for(int ipt=0; ipt<2; ipt++){
      ...
      for(int iy=0; iy<ana::nuOfY; iy++){
         ...
         calSigCent(hSignalScale, hBkg, 
               ...
               sig, s, b, sErr, bErr);
         calSigCent(hSignalMtdScale, hBkgMtd, 
               yCutMin, yCutMax, pTCutMin, pTCutMax, massCutMin, massCutMax, 
...
/*
      ...
      c1->SaveAs(Form("LamCSigAlice0-10/sig_pT%.1f_%.1f.png", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      ...
      ltx->SetTextSize(0.035);
      ltx->DrawLatexNDC(0.6, 0.3, Form("%.1f < p_{T} < %.1f GeV", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      c2->SaveAs(Form("LamCSigAlice0-10/scaled_s_pT%.1f_%.1f.png", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      ...
      ltx->SetTextSize(0.035);
      ltx->DrawLatexNDC(0.6, 0.3, Form("%.1f < p_{T} < %.1f GeV", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      c3->SaveAs(Form("LamCSigAlice0-10/scaled_b_pT%.1f_%.1f.png", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      */
   }
   ...
   hSigMtdVsPt->GetYaxis()->SetRangeUser(0, max*1.3);
   ...
   ltx->SetTextSize(0.035);
   ltx->DrawLatexNDC(0.6, 0.3, "|y|<3");
   c3->SaveAs("DB0_10.png");

   std::cout << "w/o mtd" << std::endl;
   for(int ipt=0; ipt<ana::nuOfPt; ipt++){
      std::cout << hSigVsPt->GetBinContent(ipt+1) << Form("  // %.1f - %.1f", ana::ptbin[ipt], ana::ptbin[ipt+1]) <<std::endl;;
   }
   std::cout << "w/ mtd" << std::endl;
   for(int ipt=0; ipt<ana::nuOfPt; ipt++){
      std::cout << hSigMtdVsPt->GetBinContent(ipt+1) << Form("  // %.1f - %.1f", ana::ptbin[ipt], ana::ptbin[ipt+1]) <<std::endl;;
   }
}
