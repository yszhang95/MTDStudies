...
void dlSig3DCut()
{
   ...
   f3->GetObject("hdlSig3DVsPtVsY", hSignal);
   f4->GetObject("hdlSig3DVsPtVsY", hBkg);
   f3->GetObject("hdlSig3DVsPtVsYMtd", hSignalMtd);
   f4->GetObject("hdlSig3DVsPtVsYMtd", hBkgMtd);

   // try to scale gen spectrum
   std::vector<double> scale_factor_perEvt(ana::nuOfPt);

   TH3D* hScale = new TH3D("hScale", "", ana::nyAbs, ana::yAbsMin, ana::yAbsMax, ana::npt, ana::ptMin, ana::ptMax, ana::ndlSig3D, ana::dlSig3DMin, ana::dlSig3DMax);

   calScalePerEvt(hGenPtMidY, hData, hScale, scale_factor_perEvt, ana::ndlSig3D);
   ...
   *hSignalMtdScale = (*hSignalMtd) * (*hScale);
   ...
   for(int ipt=0; ipt<ana::nuOfPt; ipt++){
      ...
      for(int iy=0; iy<ana::nuOfY; iy++){
         ...
         hSig[ipt][iy] = new TH1D(Form("hSigPt%dY%d", ipt, iy), "", ana::ndlSig3D, ana::dlSig3DMin, ana::dlSig3DMax);
         hSigMtd[ipt][iy] = new TH1D(Form("hSigMtdPt%dY%d", ipt, iy), "", ana::ndlSig3D, ana::dlSig3DMin, ana::dlSig3DMax);

         hS[ipt][iy] = new TH1D(Form("hSPt%dY%d", ipt, iy), "", ana::ndlSig3D, ana::dlSig3DMin, ana::dlSig3DMax);
         hSMtd[ipt][iy] = new TH1D(Form("hSMtdPt%dY%d", ipt, iy), "", ana::ndlSig3D, ana::dlSig3DMin, ana::dlSig3DMax);

         hB[ipt][iy] = new TH1D(Form("hBPt%dY%d", ipt, iy), "", ana::ndlSig3D, ana::dlSig3DMin, ana::dlSig3DMax);
         hBMtd[ipt][iy] = new TH1D(Form("hBMtdPt%dY%d", ipt, iy), "", ana::ndlSig3D, ana::dlSig3DMin, ana::dlSig3DMax);

         for(int idlSig3D=0; idlSig3D<ana::ndlSig3D; idlSig3D++){
            double yCutMin = ana::ybin[iy];
            double yCutMax = ana::ybin[iy+1];

            double dlSig3DCutMin = (ana::dlSig3DMax - ana::dlSig3DMin) / (double) ana::ndlSig3D * idlSig3D + ana::dlSig3DMin;
            double dlSig3DCutMax = ana::dlSig3DMax;

            double sig, s, b, sErr, bErr;
            double sigMtd, sMtd, bMtd, sMtdErr, bMtdErr;

            calSig(hSignalScale, hBkg, 
                  yCutMin, yCutMax, pTCutMin, pTCutMax, dlSig3DCutMin, dlSig3DCutMax, 
                  sig, s, b, sErr, bErr);
            calSig(hSignalMtdScale, hBkgMtd, 
                  yCutMin, yCutMax, pTCutMin, pTCutMax, dlSig3DCutMin, dlSig3DCutMax, 
                  sigMtd, sMtd, bMtd, sMtdErr, bMtdErr);

            hSig[ipt][iy]->SetBinContent(idlSig3D+1, sig);
            hSigMtd[ipt][iy]->SetBinContent(idlSig3D+1, sigMtd);

            hS[ipt][iy]->SetBinContent(idlSig3D+1, s);
            hSMtd[ipt][iy]->SetBinContent(idlSig3D+1, sMtd);
            hS[ipt][iy]->SetBinError(idlSig3D+1, sErr);
            hSMtd[ipt][iy]->SetBinError(idlSig3D+1, sMtdErr);

            hB[ipt][iy]->SetBinContent(idlSig3D+1, b);
            hBMtd[ipt][iy]->SetBinContent(idlSig3D+1, bMtd);
         ...
         hSigMtd[ipt][iy]->GetXaxis()->SetTitle("3D decayLength Significance");
         ...
         c1->SaveAs(Form("optimizationLamC/dlSig3D/y%d/sig_pT%.1f_%.1f.png", iy, ana::ptbin[ipt], ana::ptbin[ipt+1]));
         ...
         hS[ipt][iy]->GetXaxis()->SetTitle("3D decayLength Significance");
         ...
         ltx->DrawLatexNDC(0.6, 0.3, Form("%.1f < p_{T} < %.1f GeV", ana::ptbin[ipt], ana::ptbin[ipt+1]));
         ltx->DrawLatexNDC(0.6, 0.2, Form("%.1f < |y| < %.1f GeV", ana::ybin[iy], ana::ybin[iy+1]));
         c2->SaveAs(Form("optimizationLamC/dlSig3D/y%d/scaled_s_pT%.1f_%.1f.png", iy, ana::ptbin[ipt], ana::ptbin[ipt+1]));
         ...
         hB[ipt][iy]->GetXaxis()->SetTitle("3D decayLength Significance");
         ...
         ltx->DrawLatexNDC(0.6, 0.3, Form("%.1f < p_{T} < %.1f GeV", ana::ptbin[ipt], ana::ptbin[ipt+1]));
         ltx->DrawLatexNDC(0.6, 0.2, Form("%.1f < |y| < %.1f GeV", ana::ybin[iy], ana::ybin[iy+1]));
         c3->SaveAs(Form("optimizationLamC/dlSig3D/y%d/scaled_b_pT%.1f_%.1f.png", iy, ana::ptbin[ipt], ana::ptbin[ipt+1]));
      }
