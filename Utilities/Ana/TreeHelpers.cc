                     ...
                     const bool isFirstGen)
{
  ...
  if (isFirstGen && sameChain) par.setTreeIdx(genIdx);
  return sameChain;
}
...
void MyNTuple::setNDau(const unsigned short ndau,
                     const unsigned short ngdau,
                     const std::initializer_list<unsigned short>& dauNGDau)
{
  nDau = ndau;
  nGDau = ngdau;
  std::vector<unsigned short> temp(dauNGDau.begin(), dauNGDau.end());
  for (unsigned short i=0; i<nDau; ++i) {
    dauHasNGDau[i] = temp[i];
  }
}
...
{
  if (!nDau) {
    std::cerr << "[ERROR] MyNTuple::nDau is zero."
      " Make sure you set number of daughters via MyNTuple::setNDau()!"
              << std::endl;
    return;
  }
  t->Branch("bestvtxT", &bestvtxT);
  t->Branch("bestvtxTerr", &bestvtxTerr);
  // particle level
  ...
  for (unsigned short iDau=0; iDau<nDau; ++iDau) {
    ...
    if (dauHasNGDau[iDau]) {
      ...
      t->Branch(Form("trk_dau%d_dEdx_dedxPixelHarmonic2T40", iDau), &trk_dau_dEdx_dedxPixelHarmonic2T40[iDau]);
      t->Branch(Form("trk_dau%d_dEdx_dedxPixelMeanT40", iDau), &trk_dau_dEdx_dedxPixelMeanT40[iDau]);
      ...
      t->Branch(Form("trk_dau%d_beta", iDau), &trk_dau_beta[iDau]);
      t->Branch(Form("trk_dau%d_gammaSq", iDau), &trk_dau_gammaSq[iDau]);
      t->Branch(Form("trk_dau%d_massSq", iDau), &trk_dau_massSq[iDau]);
      t->Branch(Form("trk_dau%d_pathLength", iDau), &trk_dau_pathLength[iDau]);
      t->Branch(Form("trk_dau%d_tMTD", iDau), &trk_dau_tMTD[iDau]);
      t->Branch(Form("trk_dau%d_tMTDErr", iDau), &trk_dau_tMTDErr[iDau]);
    }
  ...
  for (unsigned short iGDau=0; iGDau<nGDau; ++iGDau) {
    ...
    t->Branch(Form("trk_gdau%d_dEdx_dedxPixelHarmonic2T40", iGDau), &trk_gdau_dEdx_dedxPixelHarmonic2T40[iGDau]);
    t->Branch(Form("trk_gdau%d_dEdx_dedxPixelMeanT40", iGDau), &trk_gdau_dEdx_dedxPixelMeanT40[iGDau]);
    ...
    t->Branch(Form("trk_gdau%d_beta", iGDau), &trk_gdau_beta[iGDau]);
    t->Branch(Form("trk_gdau%d_gammaSq", iGDau), &trk_gdau_gammaSq[iGDau]);
    t->Branch(Form("trk_gdau%d_massSq", iGDau), &trk_gdau_massSq[iGDau]);
    t->Branch(Form("trk_gdau%d_pathLength", iGDau), &trk_gdau_pathLength[iGDau]);
    t->Branch(Form("trk_gdau%d_tMTD", iGDau), &trk_gdau_tMTD[iGDau]);
    t->Branch(Form("trk_gdau%d_tMTDErr", iGDau), &trk_gdau_tMTDErr[iGDau]);
...
bool MyNTuple::retrieveTreeInfo(ParticleTreeMC2& p, Long64_t it)
{
  this->bestvtxT    = p.bestvtxT();
  this->bestvtxTerr = p.bestvtxTerr();
  ...
  for (size_t iDau=0; iDau<dauIdxs.size(); ++iDau) {
    ...
    for (size_t iGDau=0; iGDau<gDauIdxs.size(); ++iGDau) {
      ...
      this->trk_gdau_dEdx_dedxPixelHarmonic2T40[gDauOffset] = p.trk_dEdx_dedxPixelHarmonic2T40().at(trkIdx);
      this->trk_gdau_dEdx_dedxPixelMeanT40[gDauOffset] = p.trk_dEdx_dedxPixelMeanT40().at(trkIdx);
      this->trk_dau_beta[gDauOffset] = p.trk_beta().at(trkIdx);
      this->trk_dau_gammaSq[gDauOffset] = p.trk_gammaSq().at(trkIdx);
      this->trk_dau_massSq[gDauOffset] = p.trk_massSq().at(trkIdx);
      this->trk_dau_pathLength[gDauOffset] = p.trk_pathLength().at(trkIdx);
      this->trk_dau_tMTD[gDauOffset] = p.trk_tMTD().at(trkIdx);
      this->trk_dau_tMTDErr[gDauOffset] = p.trk_tMTDErr().at(trkIdx);
    ...
    if (gDauIdxs.empty()) {
      ...
      this->trk_dau_dEdx_dedxPixelHarmonic2T40[iDau] = p.trk_dEdx_dedxPixelHarmonic2T40().at(trkIdx);
      this->trk_dau_dEdx_dedxPixelMeanT40[iDau] = p.trk_dEdx_dedxPixelMeanT40().at(trkIdx);
      this->trk_dau_beta[iDau] = p.trk_beta().at(trkIdx);
      this->trk_dau_gammaSq[iDau] = p.trk_gammaSq().at(trkIdx);
      this->trk_dau_massSq[iDau] = p.trk_massSq().at(trkIdx);
      this->trk_dau_pathLength[iDau] = p.trk_pathLength().at(trkIdx);
      this->trk_dau_tMTD[iDau] = p.trk_tMTD().at(trkIdx);
      this->trk_dau_tMTDErr[iDau] = p.trk_tMTDErr().at(trkIdx);
    }
  }
...
void MyGenNTuple::initNTuple()
{
  if (!nDau) {
    std::cerr << "[ERROR] MyGenNTuple::nDau is zero."
      " Make sure you set number of daughters via MyGenNTuple::setNDau()!"
              << std::endl;
    return;
  }
  // particle level
  ...
  for (unsigned short iDau=0; iDau<nDau; ++iDau) {
    t->Branch(Form("gen_etaDau%d", iDau), &gen_etaDau[iDau]);
    t->Branch(Form("gen_massDau%d", iDau), &gen_massDau[iDau]);
    t->Branch(Form("gen_pTDau%d", iDau), &gen_pTDau[iDau]);
    t->Branch(Form("gen_phiDau%d", iDau), &gen_phiDau[iDau]);
    for (unsigned short iGDau=0; iGDau<dauHasNGDau[iDau]; ++iGDau) {
      t->Branch(Form("gen_etaDau%d_GDau%d", iDau, iGDau), &gen_etaDau_GDau[iDau][iGDau]);
      t->Branch(Form("gen_massDau%d_GDau%d", iDau, iGDau), &gen_massDau_GDau[iDau][iGDau]);
      t->Branch(Form("gen_pTDau%d_GDau%d", iDau, iGDau), &gen_pTDau_GDau[iDau][iGDau]);
      t->Branch(Form("gen_phiDau%d_GDau%d", iDau, iGDau), &gen_phiDau_GDau[iDau][iGDau]);
    }
  }
}
...
bool  MyGenNTuple::retrieveTreeInfo(ParticleTreeMC2& p, const Particle& par)
{
  const auto& idx = par.treeIdx();

  const auto p4 = getGenP4(idx, p);
  ...
  for (size_t iDau=0; iDau<nDau; iDau++) {
    const auto& d = par.daughter(iDau);
    const auto dIdx = d->treeIdx();
    const auto dp4 = getGenP4(dIdx, p);
    gen_etaDau[iDau] = dp4.Eta();
    gen_massDau[iDau] = dp4.M();
    gen_pTDau[iDau] = dp4.Pt();
    gen_phiDau[iDau] = dp4.Phi();
    for (size_t iGDau=0; iGDau<dauHasNGDau[iDau]; iGDau++) {
      const auto& gd = d->daughter(iGDau);
      const auto gdIdx = gd->treeIdx();
      const auto gdp4 = getGenP4(gdIdx, p);
      gen_etaDau_GDau[iDau][iGDau] = gdp4.Eta();
      gen_pTDau_GDau[iDau][iGDau] = gdp4.Pt();
      gen_massDau_GDau[iDau][iGDau] = gdp4.M();
      gen_phiDau_GDau[iDau][iGDau] = gdp4.Phi();
    }
  }
  return true;
}
