...
void plotRaaMBCMS()
{
  ...
  double d0_sig[] = {
    //33.4763, 35.7503, 28.175, //0.0 < pT < 0.5
    1.4*39.6075, 1.4*50.8403, 1.4*41.1131, //0.5 < pT < 1.0
    109.103, 96.1365, 66.1042, //1.0 < pT < 2.0
    596.578, 244.361, 110.777, //2.0 < pT < 3.0
    1053.9, 292.921, 121.179, //3.0 < pT < 4.0
    766.539, 222.646, 92.0938, //4.0 < pT < 5.0
    606.887, 190.867, 81.8142, //5.0 < pT < 6.0
    852.826, 364.879, 128.776, //6.0 < pT < 8.0
    1378.39, 227.685, 93.3539, //8.0 < pT < 10.0
  };

  double d0_sig_womtd[] = 
  {
     //16.3071, 32.1372, 31.9969, //0.0 < pT < 0.5
    1.4*22.1004, 1.4*44.4248, 1.4*41.981, //0.5 < pT < 1.0
    66.7046, 83.1912, 63.0403, //1.0 < pT < 2.0
    408.891, 213.742, 108.062, //2.0 < pT < 3.0
    783.465, 257.008, 122.056, //3.0 < pT < 4.0
    545.438, 197.282, 99.1153, //4.0 < pT < 5.0
    468.188, 160.34, 87.4606, //5.0 < pT < 6.0
    1077.35, 312.4, 142.203, //6.0 < pT < 8.0
    1708.89, 204.145, 94.1447, //8.0 < pT < 10.0
  };
  ...
  TFile infile("Latest/sig_MB.root");
  ...
  leg->SetHeader("Cent. 0-100%");
  leg->SetBorderSize(0);
  leg->SetFillStyle(0);
  leg->SetTextSize(0.05);
  leg->AddEntry(Ds_CMS[0].get(), "D_{S}^{#pm}, MTD", "p");
  leg->AddEntry(Ds_CMSNoMTD[0].get(), "D_{S}^{#pm}, no MTD", "p");
  leg->AddEntry(gd0[0], "D^{0}/#bar{D^{0}}, MTD", "p");
  leg->AddEntry(gd0_womtd[0], "D^{0}/#bar{D^{0}}, no MTD", "p");
  leg->Draw();
  ...
  c->Print("plots/RaaMBCMS.pdf");
  c->Print("plots/RaaMBCMS.png");
}
