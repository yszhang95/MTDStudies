...
void fillHyJetsMass(std::string list="", 
    const float nRMSBTL=1.0, const float nRMSETL=1.0, 
    const bool smearT=false,
    const bool inEff = true,
    const float res=0.035, const bool disableETL=false,
    const bool forceDisable=true)
{
   ...
   for(Long64_t ientry=0; ientry<nentries; ientry++){
      ...
      if(dau1.isMtd){
        is1sigmaPionDau1 = std::fabs(1./dau1.beta_PV - ana::invBetaPion(pD1) - ana::meanPion(t, 1)) < nRMSETL * dInvBetaCut1;
        is1sigmaKaonDau1 = std::fabs(1./dau1.beta_PV - ana::invBetaKaon(pD1) - ana::meanKaon(t, 1)) < nRMSETL * dInvBetaCut1;
        is1sigmaProtonDau1 = std::fabs(1./dau1.beta_PV - ana::invBetaProton(pD1) - ana::meanProton(t, 1)) < nRMSETL * dInvBetaCut1;
        if(!isETL(t->EtaD1)) {
          is1sigmaPionDau1 = std::fabs(1./dau1.beta_PV - ana::invBetaPion(pD1) - ana::meanPion(t, 1)) < nRMSBTL * dInvBetaCut1;
          is1sigmaKaonDau1 = std::fabs(1./dau1.beta_PV - ana::invBetaKaon(pD1) - ana::meanKaon(t, 1)) < nRMSBTL * dInvBetaCut1;
          is1sigmaProtonDau1 = std::fabs(1./dau1.beta_PV - ana::invBetaProton(pD1) - ana::meanProton(t, 1)) < nRMSBTL * dInvBetaCut1;
        }
      }
      if(dau2.isMtd){
        is1sigmaPionDau2 = std::fabs(1./dau2.beta_PV - ana::invBetaPion(pD2) - ana::meanPion(t, 2)) < nRMSETL * dInvBetaCut2;
        is1sigmaKaonDau2 = std::fabs(1./dau2.beta_PV - ana::invBetaKaon(pD2) - ana::meanKaon(t, 2)) < nRMSETL * dInvBetaCut2;
        is1sigmaProtonDau2 = std::fabs(1./dau2.beta_PV - ana::invBetaProton(pD2) - ana::meanProton(t, 2)) < nRMSETL * dInvBetaCut2;
        if(!isETL(t->EtaD2)) {
          is1sigmaPionDau2 = std::fabs(1./dau2.beta_PV - ana::invBetaPion(pD2) - ana::meanPion(t, 2)) < nRMSBTL * dInvBetaCut2;
          is1sigmaKaonDau2 = std::fabs(1./dau2.beta_PV - ana::invBetaKaon(pD2) - ana::meanKaon(t, 2)) <  nRMSBTL * dInvBetaCut2;
          is1sigmaProtonDau2 = std::fabs(1./dau2.beta_PV - ana::invBetaProton(pD2) - ana::meanProton(t, 2)) < nRMSBTL * dInvBetaCut2;
        }
      }
      if(dau3.isMtd){
        is1sigmaPionDau3 = std::fabs(1./dau3.beta_PV - ana::invBetaPion(pD3) - ana::meanPion(t, 3)) < nRMSETL * dInvBetaCut3;
        is1sigmaKaonDau3 = std::fabs(1./dau3.beta_PV - ana::invBetaKaon(pD3) - ana::meanKaon(t, 3)) < nRMSETL * dInvBetaCut3;
        is1sigmaProtonDau3 = std::fabs(1./dau3.beta_PV - ana::invBetaProton(pD3) - ana::meanProton(t, 3)) < nRMSETL * dInvBetaCut3;
        if(!isETL(t->EtaD3)) {
          is1sigmaPionDau3 = std::fabs(1./dau3.beta_PV - ana::invBetaPion(pD3) - ana::meanPion(t, 3)) < nRMSBTL * dInvBetaCut3;
          is1sigmaKaonDau3 = std::fabs(1./dau3.beta_PV - ana::invBetaKaon(pD3) - ana::meanKaon(t, 3)) < nRMSBTL * dInvBetaCut3;
          is1sigmaProtonDau3 = std::fabs(1./dau3.beta_PV - ana::invBetaProton(pD3) - ana::meanProton(t, 3)) < nRMSBTL * dInvBetaCut3;
        }
      }
   ...
   TString nrms = TString::Format("%.1fBTL_%.1fETL", nRMSBTL, nRMSETL);
   const char* postfix = disableETL ? Form("_%s_NoETL.root", list.c_str()) :
     Form("_%s_%0.fps_%sLayer_%sRMS.root", list.c_str(), res * 1000, inEff ? "1" : "2",
         nrms.Replace(1, 1, "p").Replace(8, 1, "p").Data());
   if (!smearT) postfix = Form("_%s_TDR_%sRMS.root", list.c_str(), nrms.Data());
   TFile fout(Form("HyJetsMassHists_reRECO%s", postfix), "recreate");
   hMassVsPtVsY->Write();
