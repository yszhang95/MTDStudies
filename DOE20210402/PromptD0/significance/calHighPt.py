...
def calSigY(df, minPt, maxPt, iy):
  S = np.sum(df["S%d" % iy][minPt:maxPt])
  SErr = np.sum(df["SErr%d" % iy][minPt:maxPt])
  B = np.sum(df["B%d" % iy][minPt:maxPt])
  BErr = np.sum(df["BErr%d" % iy][minPt:maxPt])
  return S, SErr, B, BErr, S/np.sqrt(S + B)
...
def calSigY3(df, minPt, maxPt):
  S = np.sum(df["S0"][minPt:maxPt]) + np.sum(df["S1"][minPt:maxPt]) + np.sum(df["S2"][minPt:maxPt])
  SErr = np.sqrt( np.sum(df["SErr0"][minPt:maxPt])**2 + np.sum(df["SErr1"][minPt:maxPt])**2 + np.sum(df["SErr2"][minPt:maxPt])**2)
  B = np.sum(df["B0"][minPt:maxPt]) + np.sum(df["B1"][minPt:maxPt]) + np.sum(df["B2"][minPt:maxPt])
  BErr = np.sqrt( np.sum(df["BErr0"][minPt:maxPt])**2 + np.sum(df["BErr1"][minPt:maxPt])**2 + np.sum(df["BErr2"][minPt:maxPt])**2)
  #return S/np.sqrt(S + B)
  return S, SErr, B, BErr, S/np.sqrt(S + B)
...
if __name__ == "__main__":
  ...
  print 'v1 pT > 1'
  print 'S, SErr, B, BErr, Sig'
  print 'wMTD'
  print calSigY(wMTD, 2, 10, 0)
  print calSigY(wMTD, 2, 10, 1)
  print calSigY(wMTD, 2, 10, 2)
  print 'woMTD'
  print calSigY(woMTD, 2, 10, 0)
  print calSigY(woMTD, 2, 10, 1)
  print calSigY(woMTD, 2, 10, 2)

  print 'v1 pT > 0'
  print 'S, SErr, B, BErr, Sig'
  print 'wMTD'
  print calSigY(wMTD, 0, 10, 0)
  print calSigY(wMTD, 0, 10, 1)
  print calSigY(wMTD, 0, 10, 2)
  print 'woMTD'
  print calSigY(woMTD, 0, 10, 0)
  print calSigY(woMTD, 0, 10, 1)
  print calSigY(woMTD, 0, 10, 2)
