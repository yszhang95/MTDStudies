...
void vtxProbCut()
{
   ...
   ana::calScalePerEvt(hGenPtMidY, hData, hScale, scale_factor_perEvt, ana::nVtxProb);
   ...
   *hSignalMtdScale = (*hSignalMtd) * (*hScale);
   ...
   for(int ipt=0; ipt<1; ipt++){
      ...
      for(int iy=0; iy<ana::nuOfY; iy++){
         ...
         c1->SaveAs(Form("optimization/vtxProb/y%d/sig_pT%.1f_%.1f.png", iy, ana::ptbin[ipt], ana::ptbin[ipt+1]));
         ...
         ltx->DrawLatexNDC(0.6, 0.3, Form("%.1f < p_{T} < %.1f GeV", ana::ptbin[ipt], ana::ptbin[ipt+1]));
         ltx->DrawLatexNDC(0.6, 0.2, Form("%.1f < |y| < %.1f GeV", ana::ybin[iy], ana::ybin[iy+1]));
         c2->SaveAs(Form("optimization/vtxProb/y%d/scaled_s_pT%.1f_%.1f.png", iy, ana::ptbin[ipt], ana::ptbin[ipt+1]));
