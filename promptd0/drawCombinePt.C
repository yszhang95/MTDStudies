...
void drawCombinePt()
{
   ...
   hGenPtMidY->Scale(scale_factor_perEvt);

   TH1F* hdraw = new TH1F("hdraw", "", ana::nuOfPt, ana::ptbin);
   hdraw->SetBinContent(1, hGenPtMidY->Integral(1, 5)/(0.5*ana::TAA0_100*ana::BR * 2 ) * 1e9); // 1e9 for converting mb to pb
   hdraw->SetBinContent(2, hGenPtMidY->Integral(6, 10)/(0.5*ana::TAA0_100*ana::BR * 2 ) * 1e9); // 1e9 for converting mb to pb
   hdraw->SetBinContent(3, hGenPtMidY->Integral(10, 15)/(0.5*ana::TAA0_100*ana::BR * 2 ) * 1e9); // 1e9 for converting mb to pb
   hdraw->SetBinContent(4, hGenPtMidY->Integral(15, 20)/(0.5*ana::TAA0_100*ana::BR * 2 ) * 1e9); // 1e9 for converting mb to pb
   ...
   ltx->DrawLatexNDC(0.64, 0.46, "MB events");
   ltx->DrawLatexNDC(0.64, 0.36, "-1 < y < 1");
   //std::cout <<hGenPtMidY->Integral(20, 30)/(ana::TAA0_100*ana::BR * 2 ) * 1e9  << std::endl;
