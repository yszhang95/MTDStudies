...
ST smearBeta(TRandom& r, const ST& tinfo, const bool inEff)
{
...
void fillHyJetsMass(std::string list="", const bool inEff = true)
{
   ...
   TFile fout(Form("HyJetsMassHists_reRECO_%s_1layer_inEff%s.root",
         list.c_str(), inEff ? "True" : "False"), "recreate");
   hMassVsPtVsY->Write();
