...
void calSig0_10()
{
   ...
   TFile* f5 = new TFile("hscale0_10.root");
   TFile* f6 = new TFile("hyjetsMassHists_reRECO_wPID_4950.root");

   std::ofstream fwMB("sig0_10.txt");
   std::ofstream fwoMB("sigWo0_10.txt");
   ...
   f4->GetObject("hMassVsPtVsYCent", hBkg);
   f3->GetObject("hMassVsPtVsYMtd", hSignalMtd);
   f6->GetObject("hMassVsPtVsYCentMtd", hBkgMtd);
   ...
   *hSignalMtdScale = (*hSignalMtd) * (*hScale);
   ...
   //for(int ipt=0; ipt<2; ipt++){
      ...
      for(int iy=0; iy<ana::nuOfY; iy++){
         ...
         calSigCent(hSignalScale, hBkg, 
               yCutMin, yCutMax, pTCutMin, pTCutMax, massCutMin, massCutMax, 
               sig, s, b, sErr, bErr);
         calSigCent_4751(hSignalMtdScale, hBkgMtd, 
               yCutMin, yCutMax, pTCutMin, pTCutMax, massCutMin, massCutMax, 
      ...
      c1->SaveAs(Form("LamCSigAlice0-10New/sig_pT%.1f_%.1f.png", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      c1->SaveAs(Form("LamCSigAlice0-10New/sig_pT%.1f_%.1f.pdf", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      ...
      lgds->Draw();
      ltx->SetTextSize(0.05);
      ltx->DrawLatexNDC(0.35, 0.7, "0-10 2.5B events");
      ltx->DrawLatexNDC(0.1, 0.95, "L^{PbPb}_{int} = 3 nb^{-1}  Phase II Simulation #sqrt{s} = 5.5 TeV");
      ltx->SetTextSize(0.035);
      ltx->DrawLatexNDC(0.6, 0.3, Form("%.1f #leq p_{T} < %.1f GeV", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      c2->SaveAs(Form("LamCSigAlice0-10New/scaled_s_pT%.1f_%.1f.png", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      c2->SaveAs(Form("LamCSigAlice0-10New/scaled_s_pT%.1f_%.1f.pdf", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      ...
      c3->SaveAs(Form("LamCSigAlice0-10New/scaled_b_pT%.1f_%.1f.png", ana::ptbin[ipt], ana::ptbin[ipt+1]));
      c3->SaveAs(Form("LamCSigAlice0-10New/scaled_b_pT%.1f_%.1f.pdf", ana::ptbin[ipt], ana::ptbin[ipt+1]));
   }
}
