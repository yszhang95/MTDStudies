void drawWInEff(const std::map<std::string, TGraph*>& g,
    TGraphErrors* gtdr, const int iy, const int where)
{
  std::string w;
...
void drawInOut(const std::map<std::string, TGraph*>& g, TGraphErrors* gtdr,
    const int iy, const char* layer)
{
  ...
  c->Print(Form("drawInOut_y%d_%slayer_WErr.png", iy, layer));
  delete c;
}
...
void drawWInEffWoInEff(const std::map<std::string, TGraph*>& g,
    TGraphErrors* gtdr, const int iy, const int where, const std::string layer)
{
  std::string w;
  if (where == 0) w = "InnerLost";
  if (where == 1) w = "OuterLost";
...
void drawSigWErr()
{
  ...
  for (size_t i=0; i<labels.size(); i++) {
    ...
    for (int iy=0; iy<3; iy++) {
      ...
      (graphs[iy])[labels[i]]->RemovePoint(0);
      (graphs[iy])[labels[i]]->RemovePoint(0);
      (graphs[iy])[labels[i]]->RemovePoint(0);
    }
  }
