...
void fillHyJetsMass(std::string name="", WhereLost whereLost=innerLost, const bool inEff = true)
{
   ...
   for(Long64_t ientry=0; ientry<t->GetEntries(); ientry++){
      ...
      if ( isETL( t->EtaD1 ) ) {
        ...
        if (whereLost == innerLost) {
          // if inner lost, outer with small eta should be smeared
          ...
          // if outer lost, outer with large eta should be smeared
          if ( t->EtaD1 > etaTurnPoint ) {
            beta1_PV = smearBeta(isMtdDau1, r3, inEff,
                t->isMtdDau1, t->beta1_PV,
                t->tmtd1, t->sigmatmtd1, t->t0_PV);
   ...
   TFile fout(Form("HyJetsMassHists_reRECO_%s_0p5layer_%d_inEff%s.root",
         name.c_str(), whereLost, inEff ? "True" : "False"), "recreate");
